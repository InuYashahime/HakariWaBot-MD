...
if (!fs.existsSync("./HoshinoJadiBot/" + authFolderB)) {
fs.mkdirSync("./HoshinoJadiBot/" + authFolderB, { recursive: true });
}
if (args[0]) {
fs.writeFileSync(`HoshinoJadiBot/creds.json`, Buffer.from(args[0], 'base64').toString('utf-8'))
}
const { state, saveState, saveCreds } = await useMultiFileAuthState(`./HoshinoJadiBot/${authFolderB}`);
