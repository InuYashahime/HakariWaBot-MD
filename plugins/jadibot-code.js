...
if (!fs.existsSync("./HakariJadiBot/" + authFolderB)) {
fs.mkdirSync("./HakariJadiBot/" + authFolderB, { recursive: true });
}
if (args[0]) {
fs.writeFileSync(`HakariJadiBot/creds.json`, Buffer.from(args[0], 'base64').toString('utf-8'))
}
const { state, saveState, saveCreds } = await useMultiFileAuthState(`./HakariJadiBot/${authFolderB}`);
